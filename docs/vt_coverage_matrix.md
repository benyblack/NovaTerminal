...
| Mouse reporting | ?1000/1002/1003/1006 etc | Click (`?1000`) and drag-motion (`?1002`) tracking, plus hover motion with no buttons held under any-event tracking (`?1003`, issue #269); SGR (`?1006`) and legacy X10 coordinate encodings both supported. Motion reports are coalesced to at most one per distinct terminal cell (tracked last-reported cell, reset on buffer attach/resize, pointer exit, and `?1002`/`?1003` mode changes) so a stationary-but-jittery pointer or a TUI toggling tracking modes doesn't flood the PTY | ⚠ Partial | Unit: `tests/NovaTerminal.App.Tests/Input/TerminalInputModeEncoderTests.cs`, `tests/NovaTerminal.App.Tests/Input/TerminalViewMouseMotionTests.cs`, `tests/NovaTerminal.App.Tests/MouseReportingTests.cs` | Input layer | `?1001` (highlight tracking) and urxvt (`?1015`)/pixel-position (`?1016`) mouse coordinate encodings are not implemented; only X10 and SGR (`?1006`) coordinate encodings are supported |
