#!/usr/bin/env bash
# Generate the novaterminal-bin AUR package sources from a published GitHub release.
#
...
deb_script="${NOVA_DEB_SCRIPT:-$here/../linux/build-deb.sh}"   # NOVA_DEB_SCRIPT: test seam, see test-build-arch.sh
...
if [[ -n "$source_ref" ]]; then
  ...
  echo "reading nova.desktop, nova.1, the icon and LICENSE from git ref $source_ref"
fi
...
#   * packaging/linux/nova.desktop and nova.1 stay the single source of truth for
#     both Linux packages. A copy in the AUR repo is a copy that drifts.
#   * src/NovaTerminal.App/Assets/nova_icon.png is 662 KB (and is, despite the
...
#
# That is not a convenience. Without it, CI on a branch that edits nova.desktop (or
...
desktop_sha="$(sha_of packaging/linux/nova.desktop)"
man_sha="$(sha_of packaging/linux/nova.1)"
icon_sha="$(sha_of src/NovaTerminal.App/Assets/nova_icon.png)"
...
#
# GENERATED by packaging/arch/build-arch.sh in the NovaTerminal repository.
...
_tag=@TAG@
_rid=linux-x64
_raw="https://raw.githubusercontent.com/benyblack/NovaTerminal/${_tag}"

source=("nova.desktop::${_raw}/packaging/linux/nova.desktop"
        "nova.1::${_raw}/packaging/linux/nova.1"
        "nova_icon.png::${_raw}/src/NovaTerminal.App/Assets/nova_icon.png"
        "LICENSE-${pkgver}::${_raw}/LICENSE")
source_x86_64=("NovaTerminal-${_rid}-${_tag}.tar.gz::${url}/releases/download/${_tag}/NovaTerminal-${_rid}-${_tag}.tar.gz")
...
noextract=("NovaTerminal-${_rid}-${_tag}.tar.gz")

prepare() {
  rm -rf "${srcdir}/bundle"
  mkdir -p "${srcdir}/bundle"
  bsdtar -xf "${srcdir}/NovaTerminal-${_rid}-${_tag}.tar.gz" -C "${srcdir}/bundle"
}

package() {
  install -dm755 "${pkgdir}/usr/lib/novaterminal"
  cp -a "${srcdir}/bundle/." "${pkgdir}/usr/lib/novaterminal/"

  # Debug symbols never ship: StripSymbols=true emits a ~79 MB NovaTerminal.dbg
  ...
  find "${pkgdir}/usr/lib/novaterminal" \( -name '*.pdb' -o -name '*.dbg' \) -delete
  ...
  find "${pkgdir}/usr/lib/novaterminal" -type f -exec chmod 0644 {} +
  find "${pkgdir}/usr/lib/novaterminal" -type d -exec chmod 0755 {} +
  chmod 0755 "${pkgdir}/usr/lib/novaterminal/NovaTerminal"
  ...
  done < <(find "${pkgdir}/usr/lib/novaterminal" -name '*.so*' -print0)

  install -dm755 "${pkgdir}/usr/bin"
  # /usr/bin/nova collides with python-novaclient (OpenStack), which ships its own
  # /usr/bin/nova. pacman refuses to install over it, so a user with that package
  # gets a clear file-conflict error rather than a silently shadowed command.
  ln -s /usr/lib/novaterminal/NovaTerminal "${pkgdir}/usr/bin/nova"

  install -Dm644 "${srcdir}/nova.desktop" "${pkgdir}/usr/share/applications/novaterminal.desktop"
  # Uncompressed: makepkg's zipman option gzips man pages itself. Pre-gzipping the
  # way build-deb.sh has to would leave a nova.1.gz.gz on a stock makepkg.conf.
  install -Dm644 "${srcdir}/nova.1" "${pkgdir}/usr/share/man/man1/nova.1"
  ...
  for _size in 16 32 48 64 128 256; do
    ...
    magick "${srcdir}/nova_icon.png" -resize "${_size}x${_size}" \
      "${pkgdir}/usr/share/icons/hicolor/${_size}x${_size}/apps/novaterminal.png"
    chmod 0644 "${pkgdir}/usr/share/icons/hicolor/${_size}x${_size}/apps/novaterminal.png"
  done
}
